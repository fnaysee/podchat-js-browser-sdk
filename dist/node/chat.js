...
function Chat(params) {
  ...
  chatMessaging = new _messaging["default"](Object.assign(params, {
    ...
    typeCodeOwnerId: typeCodeOwnerId,
  ...
  initAsync = function initAsync() {
    ...
    asyncClient.on('asyncReady', function () {
      ...
      }, seenIntervalPitch); //shouldReconnectCall();
    });
      ...
      switch (state.socketState) {
        case 1:
          ...
          if (state.deviceRegister && state.serverRegister) {
            chatMessaging.chatState = true; // chatMessaging.ping();
  ...
  chatMessageHandler = function chatMessageHandler(chatMessage) {
    ...
    switch (type) {
      ...
      case _constants.chatMessageVOTypes.LEAVE_THREAD:
        ...
        var participant = formatDataToMakeParticipant(messageContent, threadId);

        var localThread = _store.store.threads.get(threadId);

        if (participant.id == chatMessaging.userInfo.id && localThread) {
          _store.store.threads.remove(threadId);
        }
        ...
        _events.chatEvents.fireEvent('threadEvents', {
          ...
          result: {
            threadId: threadId,
            thread: localThread ? localThread.get() : null,
      ...
      case _constants.chatMessageVOTypes.ADD_PARTICIPANT:
        ...
        var newParticipantThread = formatDataToMakeConversation(messageContent);

        _store.store.threads.save(newParticipantThread);

        _events.chatEvents.fireEvent('threadEvents', {
          type: 'THREAD_ADD_PARTICIPANTS',
          result: {
            threadId: threadId,
            thread: _store.store.threads.get(messageContent.id).get()
          }
        });
      ...
      case _constants.chatMessageVOTypes.REMOVED_FROM_THREAD:
        var threadThatImRemovedFrom = _store.store.threads.get(threadId);

        if (threadThatImRemovedFrom) _store.store.threads.remove(threadId);

        _events.chatEvents.fireEvent('threadEvents', {
          type: 'THREAD_REMOVED_FROM',
          result: {
            threadId: threadId,
            thread: threadThatImRemovedFrom ? threadThatImRemovedFrom.get() : null
          }
        });
      ...
      case _constants.chatMessageVOTypes.REMOVE_PARTICIPANT:
        ...
        var removeParticipantThread = _store.store.threads.get(threadId);

        if (removeParticipantThread) removeParticipantThread = removeParticipantThread.get();

        _events.chatEvents.fireEvent('threadEvents', {
          type: 'THREAD_REMOVE_PARTICIPANTS',
          result: {
            threadId: threadId,
            thread: removeParticipantThread || null
          }
        });
      ...
      case _constants.chatMessageVOTypes.MUTE_THREAD:
        ...
        var mutedThread = _store.store.threads.get(threadId);
      ...
      case _constants.chatMessageVOTypes.UNMUTE_THREAD:
        ...
        var unmutedThread = _store.store.threads.get(threadId);
      ...
      case _constants.chatMessageVOTypes.UPDATE_THREAD_INFO:
        ...
        var formattedThread = formatDataToMakeConversation(messageContent);

        _store.store.threads.save(formattedThread);
      ...
      case _constants.chatMessageVOTypes.DELETE_MESSAGE:
        ...
        _store.store.threads.get(threadId).unreadCount.decrease(msgTime);
        ...
        if (messageContent.pinned) {
          var thread = _store.store.threads.get(threadId);

          if (thread) thread = thread.get();
      ...
      case _constants.chatMessageVOTypes.LAST_SEEN_UPDATED:
        ...
        if (messageContent.lastSeenMessageNanos && _store.store.threads.get(threadId)) {
          var _msgTime = parseInt(parseInt(messageContent.lastSeenMessageTime) / 1000) * 1000000000 + parseInt(messageContent.lastSeenMessageNanos);

          if (_msgTime > _store.store.threads.get(threadId).lastSeenMessageTime.get()) {
            _store.store.threads.get(threadId).unreadCount.set(messageContent.unreadCount || 0);

            _store.store.threads.get(threadId).lastSeenMessageTime.set(_msgTime);
          }
        } // chatEvents.fireEvent('threadEvents', {
        ...
        threadObject = _store.store.threads.get(threadId) ? _store.store.threads.get(threadId).get() : threadObject;
      ...
      case _constants.chatMessageVOTypes.SET_ROLE_TO_USER:
        ...
        _events.chatEvents.fireEvent('threadEvents', {
          ...
          result: {
            threadId: threadId,
            thread: srtuThread ? srtuThread.get() : null,
      ...
      case _constants.chatMessageVOTypes.REMOVE_ROLE_FROM_USER:
        ...
        _events.chatEvents.fireEvent('threadEvents', {
          ...
          result: {
            threadId: threadId,
            thread: rrfuThread ? rrfuThread.get() : null,
            admin: messageContent
          }
        });
      ...
      case _constants.chatMessageVOTypes.PIN_THREAD:
        ...
        var pinnedThread = _store.store.threads.get(threadId);

        if (pinnedThread) {
          pinnedThread.update("pin", true);
          pinnedThread = pinnedThread.get();
        } else pinnedThread = null;
      ...
      case _constants.chatMessageVOTypes.UNPIN_THREAD:
        ...
        var unPinnedThread = _store.store.threads.get(threadId);

        if (unPinnedThread) {
          unPinnedThread.update("pin", false);
          unPinnedThread = unPinnedThread.get();
        } else unPinnedThread = null;
      ...
      case _constants.chatMessageVOTypes.CHANGE_THREAD_PRIVACY:
        ...
        _store.store.threads.save(messageContent);
      ...
      case _constants.chatMessageVOTypes.LAST_MESSAGE_DELETED:
        ...
        _events.chatEvents.fireEvent('threadEvents', {
          ...
          result: {
            threadId: threadId,
            thread: _store.store.threads.get(threadOfDeletedMessage.id).get()
          }
      ...
      case _constants.chatMessageVOTypes.LAST_MESSAGE_EDITED:
        delete messageContent.unreadCount;
        var threadOfEditedMessage = formatDataToMakeConversation(messageContent);

        _store.store.threads.save(threadOfEditedMessage);

        _events.chatEvents.fireEvent('threadEvents', {
          type: 'THREAD_INFO_UPDATED',
          result: {
            threadId: threadId,
            thread: _store.store.threads.get(threadOfEditedMessage.id).get()
          }
        }); // if (fullResponseObject) {
        //     getThreads({
        //         threadIds: [messageContent.id]
      ...
      case _constants.chatMessageVOTypes.CLOSE_THREAD:
        ...
        var closedThread = _store.store.threads.get(threadId);

        if (closedThread) {
          closedThread = closedThread.get();

          _store.store.threads.remove(threadId);
        } else closedThread = null;
  ...
  newMessageHandler = function newMessageHandler(threadId, messageContent) {
    ...
    var storedThread = _store.store.threads.get(threadId);

    var lastMessageVoCopy;
    var threadObject;

    if (storedThread) {
      if (!storedThread.latestReceivedMessage.get() || storedThread.latestReceivedMessage.get() && storedThread.latestReceivedMessage.getTime() < message.time) {
    ...
    _store.store.threads.save(message.conversation);

    if (message.ownerId != chatMessaging.userInfo.id) {
      if (message.time > _store.store.threads.get(threadId).lastSeenMessageTime.get()) {
        _store.store.threads.get(threadId).unreadCount.increase();
      }
    } else {
      //If is my message set unread count to 0
      _store.store.threads.get(threadId).unreadCount.set(0);
    }

    message.conversation.unreadCount = _store.store.threads.get(threadId).unreadCount.get();
  ...
  chatEditMessageHandler = function chatEditMessageHandler(threadId, messageContent) {
    ...
    if (message.conversation) {
      ...
      message.conversation.unreadCount = _store.store.threads.get(threadId).unreadCount.get() || 0;
    } // if (fullResponseObject) {
    //     getThreads({
    //         threadIds: [threadId]
    //     }, function (threadsResult) {
  ...
  createThread = function createThread(messageContent, addFromService, showThread) {
    ...
    if (addFromService) {
      if (_store.store.threads.get(threadData.id)) {
        delete threadData.unreadCount;
        threadData.unreadCount = _store.store.threads.get(threadData.id).unreadCount.get();
      }

      _store.store.threads.save(threadData);
  ...
  getFileFromPodspace = function getFileFromPodspace(params, callback) {
    ...
    if (params.responseType === 'link') {
      var returnLink = SERVICE_ADDRESSES.PODSPACE_FILESERVER_ADDRESS + SERVICES_PATH.PODSPACE_DOWNLOAD_FILE + "?hash=".concat(params.hashCode, "&_token_=").concat(token, "&_token_issuer_=1");
      callback({
        hasError: false,
        type: 'link',
        result: returnLink
  ...
  getImageFromPodspace = function getImageFromPodspace(params, callback) {
    ...
    if (params) {
      ...
      if (params.responseType === 'link') {
        var returnLink = SERVICE_ADDRESSES.PODSPACE_FILESERVER_ADDRESS + SERVICES_PATH.PODSPACE_DOWNLOAD_IMAGE + "?hash=".concat(params.hashCode, "&_token_=").concat(token, "&_token_issuer_=1&size=").concat(params.size, "&quality=").concat(params.quality, "&crop=").concat(params.crop);
        callback({
          hasError: false,
          type: 'link',
          result: returnLink
  ...
  */
  ...
  publicized.searchContacts = function (params, callback) {
    ...
    if (params) {
      ...
      if (parseInt(params.typeCode) > 0) {
        data.typeCode = params.typeCode;
      } // data.typeCode = generalTypeCode;//params.typeCode;
  ...
  publicized.deleteCacheDatabases = function () {
    console.warn("Method deleteCacheDatabases has been deprecated and is removed from SDK");
  }; //deleteCacheDatabases;


  publicized.clearCacheDatabasesOfUser = function () {
    console.warn("Method clearCacheDatabasesOfUser has been deprecated and is removed from SDK");
